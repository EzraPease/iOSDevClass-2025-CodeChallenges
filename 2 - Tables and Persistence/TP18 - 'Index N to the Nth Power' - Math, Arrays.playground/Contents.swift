...
func toTheNthPower(array: [Int], index: Int) -> Int? {
    guard array.count > index,
          !array.isEmpty,
          index >= 0 else { return nil }
    
    let result = pow(Double(array[index]), Double(index))
    
    return Int(result)
}

print(toTheNthPower(array: [1, 2, 3, 4, 5], index: 4)!)
...
func blackDiamond(array: [Int]) -> [Int] {
    ...
    for (index, currentNumber) in array.enumerated() {
        let result = pow(Double(currentNumber), Double(index))
...
print(blackDiamond(array: [1, 2, 3, 4, 5]))
print(blackDiamond(array: []))
