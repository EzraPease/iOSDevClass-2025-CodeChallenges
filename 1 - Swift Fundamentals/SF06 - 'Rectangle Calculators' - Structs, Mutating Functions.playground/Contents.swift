...
struct Rectangle {
    ...
    func scale(_ scale: Int = 1) {
        let scaledSideA = sideA * scale
        let scaledSideB = sideB * scale
        sideA = scaledSideA
        sideB = scaledSideB
    }
        
        func areaCalculation() -> Int {
            return scaledSideA * scaledSideB
        }
        func perimeterCalculation() -> Int {
            return 2 * (scaledSideA + scaledSideB)
        }
    }
   


Rectangle(sideA: 3, sideB: 4).areaCalculation()
