...
func insertionSorting(sort arrayInput: [Int]) -> [Int] {
    guard arrayInput.count > 1 else { return arrayInput }
    
    var variableArray = arrayInput
    var result: [Int] = []
    
    for (outerIndex, value) in variableArray.enumerated() {
        for (innerIndex, innerCheck) in (outerIndex...variableArray.count) {
            if value <= innerCheck {
                result.insert(innerCheck, at: outerIndex)
                variableArray.remove(at: innerIndex)
            } else {
                continue
            }
        }
